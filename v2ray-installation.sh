...
if [ -n "$VERSION" ]; then
  ...
  CUR_VER="$(/usr/bin/v2ray -version | head -n 1 | cut -d " " -f2)"
...
VER=`ls /tmp/v2ray | grep $VDIS| cut -d - -f2`
...
cp "/tmp/v2ray/v2ray-${VER}-linux-${VDIS}/v2ray" "/usr/bin/v2ray"
...
if [ -n "${SYSTEMCTL_CMD}" ]; then
  if [ ! -f "/lib/systemd/system/v2ray.service" ]; then
    ...
    sed -i "s/v2ray\/v2ray/v2ray/g" "/lib/systemd/system/v2ray.service"
    sed -i "/Restart/a LimitNOFILE=51200" "/lib/systemd/system/v2ray.service"
    systemctl daemon-reload
  ...
  if [ ! -f "/etc/init.d/v2ray" ]; then
    ...
    sed -i "s/v2ray\/v2ray/v2ray/g" "/etc/init.d/v2ray"
